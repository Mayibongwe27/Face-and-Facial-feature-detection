...
public:
	...
	static void detectFacialFeatures(Mat& img, const vector<Rect_<int> > faces, string eye_cascade,
		string nose_cascade, string mouth_cascade,string label)
	{ 
		for (unsigned int i = 0; i < faces.size(); ++i)
		{
			...
			if (!eye_cascade.empty())
			{
				...
				{
					Rect e = eyes[j];
					rectangle(ROI, Point(e.x, e.y), Point((e.x + e.width), (e.y + e.height)), Scalar(255, 0, 0), 1, 8, 0);
				...
				for (unsigned int j = 0; j < eyes.size(); ++j)
				{
					Rect e = eyes[j];
					rectangle(ROI, Point(e.x, e.y), Point((e.x + e.width), (e.y + e.height)), Scalar(255, 0, 0), 1, 8, 0);
					circle(ROI, Point(e.x + e.width / 2, e.y + e.height / 2), 3, Scalar(0, 255, 0), -1, 8);///for dot on iris
					Mat eyee = ROI(Rect(e.x, e.y, e.width, e.height));
					imwrite("C:\\Users\\Mayibongz\\Documents\\Visual Studio 2013\\Projects\\Features\\Features\\eyes\\" + label + ".JPG", eyee);///prev error i was using the rect not the image so i edited it to include eyee
			...
			if (!nose_cascade.empty())
			{
				...
				for (unsigned int j = 0; j < nose.size(); ++j)
				{
					...
					*/
					...
					//circle(ROI, Point(n.x + n.width / 2, n.y + n.height / 2), 3, Scalar(0, 255, 0), -1, 8);
					nose_center_height = (n.y + n.height / 2);
					rectangle(ROI, Point(n.x, n.y), Point((n.x + n.width), (n.y + n.height)), Scalar(0, 255, 255), 1, 8, 0);
					Mat nos = ROI(Rect(n.x, n.y, n.width, n.height));
					///imwrite(label + ".JPG", nos);
					//imwrite("C:\\Users\\Mayibongz\\Documents\\Visual Studio 2013\\Projects\\FaceDetect\\FaceDetect\nose" + label + ".JPG", nos);
					imwrite("C:\\Users\\Mayibongz\\Documents\\Visual Studio 2013\\Projects\\Features\\Features\\nose\\" + label + ".JPG", nos);
			...
			if (!mouth_cascade.empty())
			{
				...
				for (unsigned int j = 0; j < mouth.size(); ++j)
				{
					...
					{
						rectangle(ROI, Point(m.x, m.y), Point(m.x + m.width, m.y + m.height), Scalar(0, 255, 0), 1, 4);
						Mat mouthee = ROI(Rect(m.x, m.y, m.width, m.height));
						imwrite("C:\\Users\\Mayibongz\\Documents\\Visual Studio 2013\\Projects\\Features\\Features\\mouth\\" + label + ".JPG", mouthee);
					}
					else if ((is_full_detection) && (mouth_center_height <= nose_center_height))
						continue;
					else
